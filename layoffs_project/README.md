...
Fully cleaned script: [0_data_cleaning.sql](queries/0_data_cleaning.sql)
...
View the full query here: [1_largest_layoff_event.sql](queries/1_largest_layoff_event.sql)
...
![Most Well-Funded Companies That Still Shut Down Completely](assets/1_largest_layoff_visual.png)
...
View the full query here: [2_companies_laid_off.sql](queries/2_companies_laid_off.sql)

### Results

![Companies With the Most Total Layoffs](assets/2_most_laidoff_visual.png)
...
View the full query here: [3_industries_and_countries.sql](queries/3_industries_and_countries.sql)

### Results

![Top 5 Countries by Total Layoffs](assets/3_countries_visual.png)
![Top 5 Industries by Total Layoffs](assets/3_industries_visual.png)
...
View the full query here: [4_layoffs_over_time.sql](queries/4_layoffs_over_time.sql)

### Results

![Global Tech Layoffs by Month (2020-2023)](assets/4_layoffs_over_time.png)
...
This query is designed to see which companies led the way in layoffs each individual year, as I ranked the companies within each year by total layoffs, focusing on the top 5.
...
View the full query here: [5_top_companies.sql](queries/5_top_companies.sql)
...
This query was ran to see which industries were cutting the deepest proportionally, as I calculated the average layoff percentage based on industry.
...
View the full query here: [6_layoff_severity_industry.sql](queries/6_layoff_severity_industry.sql)

### Results

![Average Layoff Severity by Industry](assets/6_avg_layoff_industry.png)
...
View the full query here: [7_layoff_distribution.sql](queries/7_layoff_distribution.sql)

### Results

![Layoff Severity Distribution](assets/7_layoff_severity_visual.png)

### Insights

- Of the **221 events** classified as severe (50%+ layoffs), **52.5%** resulted in a complete shutdown, suggesting that once layoffs cross the halfway mark, companies are more than likely to fail entirely.
- **65%** of all layoff events cut less than 25% of a company's workforce, and complete shutdowns were slightly more common than layoffs in the 50-99% range, as it suggests that companies rarely survive once cuts become severe.
...
4. **Layoffs Over Time**: A short, but sharp COVID-19 driven spike in early 2020 was followed by a large wave from the middle of 2022 through early 2023, with January 2023 accounting for the single highest monthly total within the dataset.
...
6. **Average Layoff Severity by Industry**: Aerospace had the highest average layoff severity (57%) despite not leading in total headcount, proving that industries hit hardest by volume are not always the ones hit severely proportionally.
